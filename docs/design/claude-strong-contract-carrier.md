...
per room Claude provider id as an escaped multiline TypeScript template literal, plus the contract
generation string. The readable generated form remains byte-exact after TypeScript evaluates it;
backslashes, backticks, and `${` sequences are escaped by the generator. There is therefore still
exactly one authoritative prose source (`src/room/prompts/**`); the plugin holds a generated copy in
the same sense `CLAUDE.md` does.
...
- **Blast radius on absence is degradation, never permission.** With the plugin missing, disabled,
  ...
  `disallowedTools`, role-settings `permissions.deny`, environment pins, `paseoTools`,
  role-home isolation, and `CLAUDE.md` are all untouched by plugin state.
...
- **Non-regression (integration):** `CLAUDE.md` content, provider `disallowedTools`, role-settings
  `permissions.deny`, environment pins, `paseoTools`, profiles, role-home isolation, and credential
  preserve-only behaviour are byte- and behaviour-identical with the plugin present, absent, and failed.
...
|---|---|---|---|
| Q-001 | Does a resumed Claude session re-invoke `before('agent.create')`, or does it carry a previously injected prompt? | Repository owner | answered 2026-09-23 for Paseo `0.9.1`, at the Paseo level only — both. Paseo persists the hook's `systemPrompt` in the stored agent config; `paseo agent reload` reopens the session from it without re-running the hook, and a resume after daemon restart re-runs the hook with that stored prompt as input, where the generation marker keeps the contract single (a naive appending hook doubled it in the same probe). Model ingestion on resume is not observed and `0.8.x` is not probed, so `CLAUDE.md` stays the default fallback. Revisited 2026-09-26 for `0.9.2`: resume builds the session from the stored config and never calls `before('agent.create')` (`agent-manager.js` `resumeAgentFromPersistenceInternal`); the only hook on that path is `agent.session_open`, which may change env alone. After a daemon restart, four live room seats resumed with their stored prompts and older generations. A resumed Claude seat keeps the contract it was created with, and a newer generation needs a new seat. |
...
| Q-003 | Will the plugin need re-verification against the 0.9 plugin API? | Repository owner | answered on 2026-09-23 — no code change was needed. Paseo `0.9.1` carries a byte-identical plugin compiler, unchanged creation hooks and an unchanged provider/profile config schema, and the carrier loaded to `running` on an isolated `0.9.1` daemon once the bound was widened to `<0.10.0`. The same question now stands for `0.10`. |
...
| 2026-09-19 | Bytes | Alignment with runtime hardening D8: recorded the minimal room-owned role settings and `permissions.deny` as a second Claude native-tool enforcement carrier independent of plugin state; operator settings and credentials remain outside that generated file. |
