...
| Session resumed rather than created | On `0.9.1` the stored prompt persists and a daemon-restart resume re-runs the hook, which the marker keeps idempotent (Q-001); unprobed on `0.8.x` | `CLAUDE.md` still covers it |
