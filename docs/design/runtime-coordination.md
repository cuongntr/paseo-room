...
| Not rehearsed live | Daemon restart (this session runs inside Paseo; the operator performs it); whole-room `remove --apply` (would delete the operator's real room; covered by CLI tests); visual panel check on wide/compact and light/dark (needs the operator's eyes). | pending |
...
| Q-003b | Do the server's refusal, idempotency and recovery semantics hold on every exact provider path? | Maintainer | resolved 2026-09-22 for the plugin-reload boundary on `claude-peer`, `codex-peer` and `pi-peer` (see Phase 1 release qualification); receipt replay across a daemon restart awaits the operator-run restart. Original scope: open — Phase 1 exit criterion, not a Phase 0 blocker; `handoff`, wrong-kind, failed-precondition, malformed/unknown-field, stale-generation, duplicate-identical, reused-request-ID conflict, misattributed, no-call and durable receipt replay across reload/daemon restart require the real validating server and cannot be proven by a probe that always accepts |
...
| Date | Author | Change |
|---|---|---|
| 2026-09-22 | Bytes | Recorded the Phase 1 release qualification on Paseo `0.8.0` across all three exact Peer paths, including two defects it found and the fixes (hoisted plugin entries; operator-owned `provider/model` Peer creation). Q-003b resolved up to the plugin-reload boundary; daemon restart pending. |
