...
| REQ-014 | Surface deterministic operational findings and route attention with provenance. | P2 | Duplicate Lead, missing/changed candidate, stale writer, overlapping scope, unrun gate, failed delivery, malformed call, and drifted runtime manifest each link to source events. A typed recipient matrix classifies each as record, panel, Lead-now, Supervisor-digest/now, operator-now, or Human-required; assignment-local events do not notify Supervisor by default; findings never reach the Peer they concern or become automatic technical instructions. |
...
| Phase 3 — Deterministic operational guardrails | Findings, typed recipient policy, incident deduplication and feedback, role-filtered history, export, retention/compaction with raw-event preservation, richer recovery actions, and UI refinement. | REQ-014 and REQ-021 pass; mandatory pages always deliver independently of attention budgets; assignment-local events do not notify Supervisor by default; injected duplicate, stale, malformed, delivery, gate and candidate failures remain visible; no model sensor or external telemetry is introduced. |
...
| Q-005 | What exact Paseo patch becomes the runtime server minimum after hook, lifecycle, parentage, reporting-call recovery, archive-status, singleton and RPC behavior are proven? | Maintainer | resolved 2026-09-22, widened 2026-09-23 — preview range `>=0.8.0 <0.10.0`. `0.8.0` carries the full exact-Peer evidence; `0.9.1` was admitted on host-contract evidence recorded in the design's §14 plus a load and config round-trip, without a fresh dispatch rehearsal. The exclusive upper bound reflects that `0.10.0` is unqualified. Phase 4 decides whether runtime leaves preview |
...
| Q-009 | Which Jev endpoint/SDK, secret-setting surface, retention/ZDR contract, and pinned model version are acceptable? | Repository owner + Maintainer | deferred — separate Phase 5 design delta; no key or external call enters Phase 1 through Phase 4 |
| Q-010 | What question set, evidence retention and measured thresholds justify moving a Jev sensor from shadow to assist? | Repository owner + Maintainer | deferred — require positive/negative fixtures and real `useful`/`noise`/`unknown` marks per pinned model version; vendor confidence alone is insufficient |
| Q-011 | What canonical glob grammar is sufficient for write scopes and serial-only paths without becoming a policy language? | Maintainer | deferred — resolve in the Phase 2 design delta before planning concurrency |
...
| Date | Author | Change |
|---|---|---|
| 2026-09-23 | Bytes | Widened the preview Paseo range to `>=0.8.0 <0.10.0` for Paseo `0.9.1` after proving the plugin host contract unchanged; no requirement, authority or tool surface changed. |
