...
describe('notices', () => {
  it('delivers an owner notice to Lead with its stable id in the text', async () => {
    ...
    expect(prompt).toEqual({ text: `[paseo-room notice ${noticeId}] Peer asked a question.`, messageId: noticeId });
  ...
  it('records operator notices as delivered to status without messaging any seat', async () => {
    const h = await room();
    const project = await loaded(h);
    const before = h.paseo.calls.filter(call => call.operation === 'run').length;
    ...
    expect(h.paseo.calls.filter(call => call.operation === 'run')).toHaveLength(before);
    expect((await loaded(h)).state.notices.get(noticeId)?.state).toBe('sent');
  });
